...
For a full column rank matrix $A\in\R^{p\times m}$, with $p=n+m\geq m$, the thin QR 
factorization is
...
Let $Q_2 \in \R^{p\times n}$ be a matrix whose columns form an orthonormal basis of the orthogonal
...
Observing that he term $\norm{d}^2$ is independent of $w$, we can rewrite the minimization problem like follows
\begin{equation}
\label{eq:qr_triangular_solve}
    \arg\min_{w\in\R^m} \norm{Aw-b}^2
    =
...
Since $A^TA = (Q_1R)^T(Q_1R)$, this is the QR analogue of solving the normal equations. Indeed,
...
\label{sec:qr_cost_project}
For a generic dense matrix $A\in\R^{p\times m}$, with $p=n+m\geq m$, computing a thin QR factorization requires

\begin{equation}
    O(pm^2) = O((n+m)m^2)
\end{equation}

operations. Once the factorization is available, computing the transformed right-hand side $c=Q_1^Tb$ requires at most $O(pm)$ additional operations, while solving the upper triangular system

\begin{equation}
    Rw=c
\end{equation}

by back-substitution costs $O(m^2)$.
...
which contains a cubic term in $m$.
...
Householder reflectors are the standard stable mechanism for computing QR factorizations~\cite{trefethen1997,golub2013}. A Householder reflector is an orthogonal matrix of the form
...
Here $\text{sign}(0)$ is taken as $1$. The sign choice avoids cancellation when $z_1$ is 
already close to $\norm{z}_2$, and is therefore important for numerical stability.

Thus, at step i, the Householder reflector $H$ is chosen such that the current vector $z_i$ is mapped to one whose entries below position $i$ are zero, while the i-th entry becomes $\alpha$.

\subsection{Dense Householder QR}

For a generic dense matrix $A\in\R^{p\times m}$, Householder QR proceeds column by 
column. At step $k$, one constructs a reflector $H_k$ acting on matrix $A$ only on the elements of rows $k,\ldots,p$ of the $k$-th column, such that
...
Since each Householder reflector is orthogonal, their product is also
orthogonal. Defining
...
Let $Q_1 \in \R^{p\times m}$ denote the first $m$ columns of $Q$. Since the remaining rows of the block matrix above are zero, this reduces to the thin QR factorization
...
The important implementation point is that $Q$ is never formed explicitly.
At each iteration of the factorization, the same Householder reflector applied
to $A$ is also applied to the corresponding part of the right-hand side $b$:

\begin{equation}
    b_{k:p} \leftarrow H_k b_{k:p}
    =
    b_{k:p} - \tau_k u_k \left(u_k^T b_{k:p}\right).
\end{equation}

After all $m$ reflectors have been applied, $b$ contains $Q^T b$. Its first
$m$ entries are
...
\label{sec:structured_householder} The augmented matrix has the special block structure
...
The remaining $n$ rows of $X^T$ can therefore be incorporated one at a time.
...
At step $k$, the current matrix $R$ is already upper triangular and the
...
For this reason, it is not necessary to build a Householder reflector on the
...
Thus, at each step only the pair $(R_{kk},a_k)$ is needed to determine the
reflector: the diagonal entry is updated to $\rho$, while the corresponding
entry of the inserted row is annihilated.

The same $2\times2$ reflector must then be applied to the remaining entries
...
where $\eta$ is initialized as $\eta=y_i$ at the beginning of the insertion of row $x_i^T$.

After repeating this procedure for $k=1,\ldots,m$, all entries of the inserted row have been annihilated. The row can then be discarded, while the updated $R$ and $c$ contain the effect of the newly inserted data row.
...
The structured row-insertion procedure exploits two properties of the augmented matrix: the regularization block $\lambda I_m$ is already triangular, and only the $n$ dense rows of $X^T$ need to be incorporated. For each inserted row, the algorithm performs $m$ two-dimensional Householder transformations.
At iteration $k$, the reflector is applied only to the trailing row segments \begin{equation} R_{k,k:m} \qquad\text{and}\qquad a_{k:m}, \end{equation} whose length is $m-k+1$. 
Hence, the computational cost of inserting a single row is
...
\begin{equation}
    \label{eq:structured_qr_cost} O(nm^2)
\end{equation}
operations.
Applying the same two-dimensional transformations to the right-hand side costs only $O(nm)$ additional operations and is therefore lower order.
The final triangular solve $Rw=c$ requires $O(m^2)$ operations.
The total computational cost is consequently
...
for the complete factorization and solve.
In the ML-CUP setting, where $n\ll m$ and $n$ is regarded as fixed, this is quadratic in $m$.
This should be contrasted with the generic dense QR cost derived in Section~\ref{sec:qr_cost_project},

\begin{equation}
    O((n+m)m^2) = O(nm^2+m^3).
\end{equation}

The structured implementation removes the cubic $O(m^3)$ term of a generic dense QR factorization by exploiting the already triangular block $\lambda I_m$ and processing only the $n$ dense rows of $X^T$.
Its total computational cost is therefore
...
The dominant storage cost is the upper triangular factor $R\in\R^{m\times m}$, requiring $O(m^2)$ memory, while the working vectors require only $O(m)$ additional storage. If all Householder reflectors are stored, an additional $O(nm)$ memory is required.

Finally, since the method relies only on orthogonal transformations, it retains the numerical stability of QR factorization and avoids explicitly forming $A^TA$, whose condition number satisfies $\kappa(A^TA)=\kappa(A)^2$.
