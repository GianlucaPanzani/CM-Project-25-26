...
sweep. The normal-equations timer is partial, while the row-insertion values
characterize the current defective implementation and are reported only as
timing diagnostics.}
...
\noindent Within the comparable QR timings, at $m=4000$ the valid
structure-based path takes approximately $0.494$ seconds, versus $197.121$
seconds for dense QR, a factor of about $399$. On the final doubling from
$m=2000$ to $m=4000$, the dense and structure-based times increase by factors
of approximately $6.79$ and $3.59$, respectively, which is consistent with
the cubic and quadratic terms in Eq.~\eqref{eq:qr_scaling_summary}. These six
measurements are empirical evidence, not a proof of asymptotic complexity;
moreover, the notebook computes reference-slope arrays but does not draw them.
...
\noindent Two notebook limitations remain. First, the row-insertion curve
inherits the implementation defect described above and is not a valid
stability measurement. Second, the printed diagnostics table uses the final
scalar condition number and final scalar error values in every row instead of
indexing the stored arrays, so its display is incorrect. The plots use the
stored arrays; the condition numbers, SVD reference calculations,
normal-equations results, dense-QR results, and structure-based results are
valid and support the comparison made here.
