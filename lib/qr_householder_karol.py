...
def apply_structure_based_QT(u_list, b_perm, m):
    ...
    # Copy the fully transformed tail back, so that c equals Q^T b_perm in
    # all m+n coordinates (not just the leading m used by the solve).
    c[m:] = y_transformed
    return c
...
def qr_factorize_row_insertion_2d(X, lam, y, store_reflectors=True):
    ...
    for i in range(n):
        ...
        for k in range(m):
            ...
            # Update the triangular matrix and the inserted row.
            # IMPORTANT: apply the full reflector update to the slices FIRST,
            # then overwrite the structural entries (diagonal = alpha, sub = 0).
            # Doing it in the opposite order would subtract from the values we
            # just set, corrupting them.
